...
RakitApp keeps backend action tests in isolated SQLite databases and mocks external email and file-upload services. Run `pnpm test:backend` after changing an action, server module, or shared module used by the backend. Use `pnpm test:backend:watch` while developing.

To enable the repository hook that runs the backend suite automatically before a commit, run `pnpm hooks:install` from the repository root. The same suite runs in CI when RakitApp backend files change.
