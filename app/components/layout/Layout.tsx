  ...
  const contentFrame = (
    <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
      {isChatRoute ? (
        ...
        <div
          className={`flex h-12 shrink-0 items-center gap-3 border-b px-4 md:hidden ${location.pathname === "/admin" ? "border-slate-800 bg-[#070b16]" : "border-border"}`}
        >
          <button
            type="button"
            onClick={() => setMobileSidebarOpen(true)}
            aria-label={t("navigation.openNavigation")}
            className={`flex h-9 w-9 cursor-pointer items-center justify-center rounded-md ${location.pathname === "/admin" ? "text-slate-300 hover:bg-slate-800 hover:text-white" : "text-muted-foreground hover:bg-accent hover:text-foreground"}`}
          >
